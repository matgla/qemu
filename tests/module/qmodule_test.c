...
{
    memory_region_init(&test_memory, NULL, "system", UINT64_MAX);
    address_space_init(&address_space_memory, &test_memory, "memory");
}

static void qmt_test_memory_finalize(void)
{
    object_unparent(OBJECT(&test_memory));
...
static char *qmt_print_registered_calls(const QMTExpectationSet *set)
{
    ...
    {
        const QMTRegisteredCall *call = &set->registered_calls[i];
...
// {
//     char *msg = g_strdup_printf("Expected %s(", e->name);
//     return msg;
// }
...
static char *qmt_print_call(const char* name, const QMTExpectationSet *set, int arguments_count, const QMTArgument *arguments)
{
...
static char *qmt_print_expectation(const QMTExpectation *e, const int arguments_count, const char *name)
{
    char *msg = (char *)"";
...
static void qmt_assert_expectations_msg(const char *name, const char *file, int line, 
    const QMTExpectationSet *set, int arguments_count, const QMTArgument *arguments)
{
...
static bool qmt_expectation_match(const QMTExpectation *e, const QMTArgument *arguments, const int arguments_count)
{
    for (int i = 0; i < arguments_count; ++i) 
    {
...
{
    QMTExpectationSet *set = qmt_find_expectations_for_function(func); 
    bool matched = false;
...
static bool qmt_compare_expectation_comparators(const QMTExpectation *a, const QMTExpectation *b, const int arguments_count)
{
    return qmt_compare_comparators(a->comparators, b->comparators, arguments_count);
}

static bool qmt_expectation_contains_place(const QMTExpectation *a, const QMTExpectationPlace *place)
{
...
QMTExpectation *qmt_expect_call(const char *name, const char *file, int line, void *func, int n, ...)
{
    ...
    if (set == NULL) {
        QMTExpectations *expectations = qmt_get_expectations();
        ++expectations->size;
        expectations->sets = (QMTExpectationSet *)g_realloc(expectations->sets, expectations->size * sizeof(QMTExpectationSet));
    ...
    expectation.places = (QMTExpectationPlace *)g_malloc0(sizeof(QMTExpectationPlace));
    ...
    for (int i = 0; i < set->expectations_count; ++i)
    {
        ...
        if (qmt_compare_expectation_comparators(e, &expectation, n))
        {
            ...
            {
                e->places = (QMTExpectationPlace *)g_realloc(e->places, sizeof(QMTExpectationPlace) * (e->places_count + 1));
    ...
    set->expectations = (QMTExpectation *)g_realloc(set->expectations, set->expectations_count * sizeof(QMTExpectation));
    set->expectations[set->expectations_count - 1] = expectation;
    /* Try to find expectation, if already exists, just increment expect count */
...
// static char* qmt_print_any(QMTArgument *value)
// {
//     char *out = NULL;
//     out = g_strdup_printf("any");
//     return out;
// }
...
// static bool qmt_is_null(void *lhs)
// {
//     return lhs == NULL;
// }
